<template>
  <div class="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:py-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
    <div class="mb-12">
      <h1 class="text-4xl font-serif text-slate-900 italic">{{ t('header.title') }}</h1>
      <p class="text-slate-500 mt-2 font-medium tracking-tight">{{ t('header.subtitle') }}</p>
    </div>
        ...
        <div class="relative group cursor-pointer" @click="triggerAvatarUpload">
          ...
          <div v-if="uploadingAvatar" class="absolute inset-0 bg-white/60 backdrop-blur-sm rounded-3xl flex items-center justify-center z-10">
            <div class="flex flex-col items-center">
              ...
              <span class="text-[10px] font-bold text-slate-900 mt-2 uppercase tracking-widest">{{ t('avatar.updating') }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="flex-grow space-y-12">
        ...
        <section class="space-y-8">
          <div class="border-b border-slate-100 pb-4 flex items-center justify-between">
            <h2 class="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">{{ t('section.contact_details') }}</h2>
            <div v-if="updated" class="text-emerald-500 text-[10px] font-bold uppercase animate-pulse">{{ t('msg.changes_saved') }}</div>
          </div>

          <form @submit.prevent="handleUpdateInfo" class="grid grid-cols-1 gap-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">{{ t('label.preferred_name') }}</label>
              ...
              <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">{{ t('label.phone_reference') }}</label>
            ...
            <div class="space-y-2">
              <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">{{ t('label.primary_email') }}</label>
            ...
            <div class="flex justify-start">
              <button type="submit" :disabled="updatingInfo" class="group flex items-center gap-3 text-slate-900 font-black text-xs uppercase tracking-widest">
                <span>{{ t('btn.synchronize_identity') }}</span>
        ...
        <section class="space-y-8">
          <div class="border-b border-slate-100 pb-4">
            <h2 class="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">{{ t('section.security_protocol') }}</h2>
          </div>

          <form @submit.prevent="handleUpdatePassword" class="space-y-10">
            <div class="space-y-2">
              <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">{{ t('label.current_password') }}</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">{{ t('label.new_credential') }}</label>
              ...
              <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">{{ t('label.confirm_credential') }}</label>
            ...
            <div class="flex justify-start">
              <button type="submit" :disabled="updatingPass" class="group flex items-center gap-3 text-slate-900 font-black text-xs uppercase tracking-widest">
                <span>{{ t('btn.update_credentials') }}</span>
                <div class="w-8 h-px bg-slate-900 transition-all group-hover:w-12"></div>
...
const { t } = useI18n(['customer.profile', 'common'])
...
const handleUpdatePassword = async () => {
  ...
  try {
    ...
    alert(t('alert.credentials_updated'))
  } catch (err: any) {
    alert(err.response?.data?.message || t('alert.update_failed'))
  } finally {
