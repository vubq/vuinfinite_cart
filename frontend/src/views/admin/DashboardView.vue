...
const { t } = useI18n(['admin.dashboard', 'common'])

const stats = [
  { label: 'stat.total_sales', value: '$12,840', change: '+12.5%', icon: 'sales' },
  { label: 'stat.active_orders', value: '156', change: '+3.2%', icon: 'orders' },
  { label: 'stat.new_customers', value: '42', change: '-1.4%', icon: 'customers' },
  { label: 'stat.growth_rate', value: '24.8%', change: '+5.7%', icon: 'growth' }
]
</script>
<template>
  <div class="space-y-8 max-w-[1400px] mx-auto animate-in fade-in zoom-in-[0.98] duration-700 ease-out pb-20">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div class="space-y-2.5">
        <div class="inline-flex items-center gap-2 px-3 py-1 bg-white border border-slate-200/60 rounded text-[10px] font-bold text-slate-500 uppercase tracking-widest shadow-sm">
          ...
          {{ t('header.system_optimal') }}
        </div>
        <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
          {{ t('header.welcome') }}<span class="text-emerald-600">{{ adminAuth.user?.username || 'Commander' }}</span>
        </h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <VCard class="relative overflow-hidden border-slate-200/60">
        ...
        <div class="relative z-10">
          <h3 class="text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-1">{{ t('stat.gross_revenue') }}</h3>
      ...
      <VCard v-for="stat in stats.slice(1)" :key="stat.label" class="relative overflow-hidden border-slate-200/60">
        <div class="relative z-10">
          <h3 class="text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-1">{{ t(stat.label) }}</h3>
          <div class="flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <!-- Primary Data View -->
      <VCard :title="t('card.transactions_title')" :subtitle="t('card.transactions_subtitle')" class="lg:col-span-2 border-slate-200/60">
        <template #headerAction>
          <button class="text-[12px] font-bold text-emerald-600 hover:text-emerald-700">{{ t('card.transactions_view_all') }}</button>
        </template>
        ...
        <div class="divide-y divide-slate-100/80 -mx-7 -mb-7">
          <div v-for="i in 5" :key="i" class="px-7 py-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors group">
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="text-[14px] font-bold text-slate-900 group-hover:text-emerald-700 transition-colors">{{ t('card.transactions_order') }}{{ 8040 + i }}</p>
                <p class="text-[12px] text-slate-500 font-medium mt-0.5">{{ t('card.transactions_verified') }}{{ i }}{{ t('card.transactions_ago') }}</p>
              </div>
            </div>
      ...
      <div class="space-y-6">
        ...
        <!-- Command Tools -->
        <VCard :title="t('card.quick_actions_title')" class="border-slate-200/60">
          <div class="grid grid-cols-2 gap-3 mt-2">
             <button v-for="action in ['Products', 'Promotions', 'Settings', 'Analytics']" :key="action"
                ...
                <span class="text-[11px] font-bold text-slate-600 uppercase tracking-widest group-hover/action:text-emerald-700">{{ t('action.' + action.toLowerCase()) }}</span>
             </button>
          </div>
        </VCard>
        ...
        <div class="p-6 rounded-2xl bg-slate-900 relative overflow-hidden group/alert border border-slate-800">
          ...
          <div class="flex items-center gap-2 mb-3 relative z-10">
            ...
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ t('alert.title') }}</span>
          </div>
          
          <h4 class="text-lg font-semibold text-white mb-4 relative z-10 leading-snug">{{ t('alert.message') }}<span class="text-emerald-400">{{ t('alert.location') }}</span>.</h4>
