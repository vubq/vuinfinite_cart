...
const { t } = useI18n(['customer.layout', 'common'])
</script>
      ...
      <nav class="max-w-7xl mx-auto px-6 md:px-10 h-20 flex items-center justify-between">
        <div class="flex items-center gap-12">
          ...
          <div class="hidden md:flex items-center gap-8 text-[13px] font-bold text-slate-500 uppercase tracking-widest">
            <router-link to="/" class="hover:text-emerald-600 transition-colors">{{ t('nav.collection') }}</router-link>
            <router-link to="/" class="hover:text-emerald-600 transition-colors">{{ t('nav.catalog') }}</router-link>
            <router-link to="/" class="hover:text-emerald-600 transition-colors">{{ t('nav.stories') }}</router-link>
          </div>
        </div>
        ...
        <div class="flex items-center gap-6">
          ...
          <template v-if="customerAuth.isAuthenticated">
            <div class="flex items-center gap-6">
              <router-link to="/profile" class="flex flex-col items-end group">
                ...
                <button @click.prevent.stop="customerAuth.logout()" class="text-[10px] font-bold text-slate-400 hover:text-rose-500 uppercase tracking-widest transition-colors">{{ t('btn.logout') }}</button>
              </router-link>
          ...
          <template v-else>
            <router-link to="/login" class="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors">{{ t('btn.login') }}</router-link>
    ...
    <footer class="bg-slate-50 border-t border-slate-100 py-16 px-10 mt-auto">
      <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left">
        ...
        <div class="flex flex-col gap-3 text-slate-500 text-sm font-bold uppercase tracking-widest">
           <p class="text-slate-800 mb-2">{{ t('footer.connect') }}</p>
        ...
        <div class="flex flex-col gap-3 text-slate-500 text-sm font-bold uppercase tracking-widest font-bold">
           <p class="text-slate-800 mb-2">{{ t('footer.support') }}</p>
           <a href="#" class="hover:text-emerald-600 transition-colors">{{ t('footer.contact') }}</a>
           <a href="#" class="hover:text-emerald-600 transition-colors">{{ t('footer.privacy') }}</a>
        </div>
      </div>
